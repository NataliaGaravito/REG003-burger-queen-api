...
describe('GET /users', () => {
    ...
    it('should fail with 403 when not admin', () => (fetchAsTestUser('/users')
        .then((resp) => expect(resp.status).toBe(403))));
    it('should get users', () => (fetchAsAdmin('/users')
        .then((resp) => {
    ...
    it('should get users with pagination', () => (fetchAsAdmin('/users?limit=1')
        .then((resp) => {
        expect(resp.status).toBe(200);
...
describe('GET /users/:uid', () => {
    ...
    it('should fail with 403 when not owner nor admin', () => (fetchAsTestUser(`/users/${config.adminEmail}`)
        .then((resp) => expect(resp.status).toBe(403))));
    it('should fail with 404 when admin and not found', () => (fetchAsAdmin('/users/abc@def.ghi')
        .then((resp) => expect(resp.status).toBe(404))));
    it('should get own user', () => (fetchAsTestUser('/users/test@test.test')
        .then((resp) => {
        expect(resp.status).toBe(200);
        return resp.json();
    })
        .then((json) => expect(json.email).toBe('test@test.test'))));
    it('should get other user as admin', () => (fetchAsAdmin('/users/test@test.test')
        .then((resp) => {
        expect(resp.status).toBe(200);
        return resp.json();
    })
...
describe('POST /users', () => {
    it('should respond with 400 when email and password missing', () => (fetchAsAdmin('/users', { method: 'POST' })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should respond with 400 when email is missing', () => (fetchAsAdmin('/users', { method: 'POST', body: { email: '', password: 'xxxx' } })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should respond with 400 when password is missing', () => (fetchAsAdmin('/users', { method: 'POST', body: { email: 'foo@bar.baz' } })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should fail with 400 when invalid email', () => (fetchAsAdmin('/users', { method: 'POST', body: { email: 'failemail', password: '123456' } })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should fail with 400 when invalid password', () => (fetchAsAdmin('/users', { method: 'POST', body: { email: 'email@test.tes', password: '12' } })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should create new user', () => (fetchAsAdmin('/users', {
        method: 'POST',
        body: {
    ...
    it('should create new admin user', () => (fetchAsAdmin('/users', {
        method: 'POST',
        body: {
    ...
    it('should fail with 403 when user is already registered', () => (fetchAsAdmin('/users', {
        method: 'POST',
...
describe('PUT /users/:uid', () => {
    it('should fail with 401 when no auth', () => (fetch('/users/foo@bar.baz', { method: 'PUT' })
    ...
    it('should fail with 400 when no props to update', () => (fetchAsTestUser('/users/test@test.test', { method: 'PUT' })
        .then((resp) => expect(resp.status).toBe(400))));
    it('should fail with 403 when not admin tries to change own roles', () => (fetchAsTestUser('/users/test@test.test', {
        method: 'PUT',
        body: { roles: { admin: true } },
    })
        .then((resp) => expect(resp.status).toBe(403))));
    it('should update user when own data (password change)', () => (fetchAsTestUser('/users/test@test.test', {
        method: 'PUT',
    ...
    it('should update user when admin', () => (fetchAsAdmin('/users/test@test.test', {
        method: 'PUT',
...
describe('DELETE /users/:uid', () => {
    it('should fail with 401 when no auth', () => (fetch('/users/foo@bar.baz', { method: 'DELETE' })
        .then((resp) => expect(resp.status).toBe(401))));
    it('should fail with 403 when not owner nor admin', () => (fetchAsTestUser(`/users/${config.adminEmail}`, { method: 'DELETE' })
        .then((resp) => expect(resp.status).toBe(403))));
    it('should fail with 404 when admin and not found', () => (fetchAsAdmin('/users/abc@def.ghi', { method: 'DELETE' })
        .then((resp) => expect(resp.status).toBe(404))));
