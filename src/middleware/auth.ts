...
export default (secret: string) =>
    (req: Request, resp: Response, next: NextFunction) => {
        ...
        jwt.verify(token, secret, async (err: any, decodedToken: any) => {
            ...
            if (!user) return error(400, req, resp, next);
            req.userId = user.id;
...
export const isAuthenticated = (req: Request): any => (req.auth ? true : false);

export const isAdmin = (req: Request): any => (req.userAdmin ? true : false);

export const isItSelf = (req: Request): any => {
    const { id } = req.params;
...
export const requireAuth = (req: Request, resp: Response, next: NextFunction): any =>
    !isAuthenticated(req) ? next(error(401, req, resp, next)) : next();
