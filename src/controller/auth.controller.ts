...
const generateToken = async (req:Request, res:Response, next: NextFunction) => {
    try {
        ...
        if((!email) || (!password) || (!emailRegExp) || (password.length <= 3)) error(400, req, res, next)
        const token = await authService.generateToken(String(email), String(password));
        if((token === 'User Not Found') || (token === 'Invalid Password')) error({statusCode: 404, message: token}, req, res, next)
