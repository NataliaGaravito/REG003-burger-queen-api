...
const getAllUsers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (
      fetchedUsers.pagination.prev !== null &&
      fetchedUsers.pagination.next !== null
    ) {
      return res
        .links({
          next: urlPage + fetchedUsers.pagination.next + urlLimit + limitNumber,
          prev: urlPage + fetchedUsers.pagination.prev + urlLimit + limitNumber,
          first:
            urlPage + fetchedUsers.pagination.first + urlLimit + limitNumber,
          last: urlPage + fetchedUsers.pagination.last + urlLimit + limitNumber
        })
        .status(200)
        .json(fetchedUsers.allUsers);
    } else if (fetchedUsers.pagination.prev !== null) {
      return res
        .links({
          prev: urlPage + fetchedUsers.pagination.prev + urlLimit + limitNumber,
          first:
        ...
        .status(200)
        .json(fetchedUsers.allUsers);
    } else if (fetchedUsers.pagination.next !== null) {
      return res
        .links({
          next: urlPage + fetchedUsers.pagination.next + urlLimit + limitNumber,
          first:
...
const createUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (err.code === 'P2002')
      return error(
        { statusCode: 403, message: 'Email already registered' },
        req,
        res,
        next
      );
...
const updateUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // if(updatedUser === "No data provided") return error(400, req, res, next);
    ...
    } else if (err.message.includes("Argument data is missing")){
       return error(400, req, res, next);
    }
    return error(500, req, res, next);
  }
