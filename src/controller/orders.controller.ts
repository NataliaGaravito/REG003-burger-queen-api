...
const statusAcceptable = ['pending', 'preparing', 'canceled', 'delivering', 'delivered']
...
const getOrderById = async (req:Request, res:Response, next: NextFunction) => {
    ...
    try {
        ...
        if(err.code === 'P2025') error({statusCode: 404, message: err.meta.cause}, req, res, next);
        return error({statusCode: 500, message: err.meta.cause}, req, res, next);
    }
}
...
const createOrder = async (req:Request, res:Response, next: NextFunction) => {
    ...
    if((status) && (!statusAcceptable.includes(status)))  error({statusCode: 400, message: "Bad status provided"}, req, res, next);
...
const updateOrder = async (req:Request, res:Response, next: NextFunction) => {
    ...
    try {
        const updatedOrder = await ordersServices.updateOrder(Number(idNumber), req.body);
